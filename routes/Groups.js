...
const authorization = require('../middleware/authorization'); // authenticated sera utiliser pour définir une authentification afin d'acceder à une route
...
module.exports.all = async function (ctx) {
    ...
    for await (let group of allGroups) {
      arrayGroups.push(new Groups(group.name, await Users.findUsersInGroups(group.id)));
    }
...
module.exports.find = async function (ctx) {
  ...
  if (group){
    ctx.body = new Groups(group.name, await Users.findUsersInGroups(ctx.params.id));
  } else {
...
module.exports.findGroup = async function (id){
  ...
  if (group){
    return new Groups(group.name, await Users.findUsersInGroups(group.id))
  }
}
